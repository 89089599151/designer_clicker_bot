...
def _resolve_database_url() -> str:
    """Return preferred database URL prioritising PostgreSQL when available."""
    ...
    pg_url = os.getenv(
        "POSTGRES_URL",
        "postgresql+asyncpg://designer:designer@localhost:5432/designer_clicker_bot",
    )
    try:
        import asyncpg  # type: ignore  # pragma: no cover - optional dependency

        return pg_url
    except Exception:
        logging.getLogger(__name__).warning(
            "asyncpg unavailable, falling back to SQLite database",
        )
        return "sqlite+aiosqlite:///./designer.db"
...
ACHIEVEMENT_RUB_REWARD = 50
ACHIEVEMENT_XP_REWARD = 100
ECONOMY_RETENTION_DAYS = 60
ECONOMY_PRUNE_INTERVAL_SECONDS = 6 * 60 * 60


_LAST_ECONOMY_PRUNE: float = 0.0
...
class RU:
    ...
    TOO_FAST = "⏳ Темп слишком высокий: {current}/{limit} кликов/сек. Дождитесь восстановления лимита."
    ...
    ACHIEVEMENT_UNLOCK_REWARD = (
        "🏆 Новое достижение: {title}! Награда: +{rub}₽ и +{xp} XP."
    )
    ACHIEVEMENTS_TITLE = "🏆 Достижения"
    ACHIEVEMENTS_EMPTY = "Пока нет достижений — продолжайте играть!"
    ACHIEVEMENTS_ENTRY = "{icon} {name} — {desc}"
    FALLBACK_HELP = "🤖 Я не понял сообщение. Нажмите кнопку ниже, чтобы вернуться в меню."
    ITEM_AUTO_EQUIPPED = "🧩 Предмет куплен и сразу экипирован!"
    ITEM_EQUIP_PROMPT = "🧩 Предмет куплен и ждёт вас в гардеробе."
...
class EconomyLog(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    __table_args__ = (
        Index("ix_economy_user_created", "user_id", "created_at"),
        Index("ix_economy_type", "type"),
    )
...
async def process_offline_income(
    ...
    await maybe_prune_economy_log(session)
...
async def maybe_prune_economy_log(session: AsyncSession) -> int:
    """Purge stale economy log records on a timed interval to limit table growth."""

    global _LAST_ECONOMY_PRUNE
    now = time.time()
    if (now - _LAST_ECONOMY_PRUNE) < ECONOMY_PRUNE_INTERVAL_SECONDS:
        return 0
    _LAST_ECONOMY_PRUNE = now
    threshold = utcnow() - timedelta(days=ECONOMY_RETENTION_DAYS)
    result = await session.execute(
        delete(EconomyLog).where(EconomyLog.created_at < threshold)
    )
    removed = result.rowcount or 0
    if removed:
        logger.info("Economy log pruned", extra={"removed": int(removed)})
    return removed
...
async def evaluate_achievements(
    ...
    for ach in achievements:
        ...
        if progress_value >= ach.threshold:
            ...
            if just_unlocked:
                reward_time = utcnow()
                user.balance += ACHIEVEMENT_RUB_REWARD
                user.updated_at = reward_time
                session.add(
                    EconomyLog(
                        user_id=user.id,
                        type="achievement_reward",
                        amount=ACHIEVEMENT_RUB_REWARD,
                        meta={"achievement": ach.code},
                        created_at=reward_time,
                    )
                )
                await add_xp_and_levelup(user, ACHIEVEMENT_XP_REWARD)
...
async def notify_new_achievements(
    ...
    if not unlocked:
        return
    lines = [
        RU.ACHIEVEMENT_UNLOCK_REWARD.format(
            title=f"{ach.icon} {ach.name}",
            rub=ACHIEVEMENT_RUB_REWARD,
            xp=ACHIEVEMENT_XP_REWARD,
        )
...
class HandlerContext:
    """State shared across message handlers for user/session management."""
    ...
    async def ensure_stats(self) -> dict:
        """Lazily resolve aggregated user stats and cache the result."""

        if self.user is None:
            return {}
        if self._stats is None:
            self._stats = await get_user_stats(self.session, self.user)
        return self._stats

    async def flush_achievements(self, message: Message) -> None:
        """Send any pending achievement notifications immediately."""

        if not self.achievements or self.user is None:
            return
        await notify_new_achievements(message, self.achievements)
...
async def interaction_context(
    ...
    *,
    ...
    preload_stats: bool = False,
) -> AsyncIterator[HandlerContext]:
    """Yield a prepared handler context with user, session, and achievements list."""

    async with session_scope() as session:
        user = await ensure_user_loaded(session, message)
        achievements: List[Tuple[Achievement, UserAchievement]] = []
        ctx = HandlerContext(session=session, user=user, achievements=achievements)
        if not user:
        ...
        if preload_stats:
            ctx._stats = await get_user_stats(session, user)
        try:
            yield ctx
        finally:
            if ctx.user:
                await notify_new_achievements(message, ctx.achievements)
            ctx.achievements.clear()
...
async def fetch_income_leaderboard(
    session: AsyncSession, user_id: int, limit: int = 5
) -> Tuple[List[Tuple[str, float]], Optional[int], int]:
    """Return top earners, the requesting user's rank, and total players."""
    ...
    total_expr = func.coalesce(income_agg.c.passive_total, 0.0) + func.coalesce(
        income_agg.c.active_total, 0.0
    )
    ranked = (
        select(
            ...
            func.rank().over(order_by=total_expr.desc()).label("rank"),
            func.count().over().label("player_count"),
        )
    ...
    raw_rows = (
        await session.execute(
            select(
                ...
                ranked.c.total,
                ranked.c.rank,
                ranked.c.player_count,
            )
            .order_by(ranked.c.total.desc())
            .limit(limit)
        )
    ).all()
    top_rows: List[Tuple[str, float]] = []
    total_players = 0
    for uid, name, total, _, player_count in raw_rows:
        display_name = name or f"Игрок {uid}"
        top_rows.append((display_name, float(total or 0.0)))
        total_players = int(player_count or total_players)

    player_row = (
        await session.execute(
            select(ranked.c.rank, ranked.c.player_count).where(ranked.c.user_id == user_id)
        )
    ).first()
    player_rank = int(player_row.rank) if player_row and player_row.rank is not None else None
    if player_row and player_row.player_count is not None:
        total_players = int(player_row.player_count)

    return top_rows, player_rank, total_players
...
class RateLimiter:
    ...
    def allow(self, user_id: int, limit_per_sec: int, now: Optional[float] = None) -> Tuple[bool, int]:
        """Return allowance flag and current events count in the 1-second window."""
    ...
    async def __call__(self, handler, event: Message, data):
        try:
            if isinstance(event, Message) and (event.text or "") == RU.BTN_CLICK:
                ...
                allowed, current = self.limiter.allow(tg_id, limit)
                if not allowed:
                    logger.debug(
                        "Rate limit hit",
                        extra={"tg_id": tg_id, "limit": limit, "current": current},
                    )
                    await event.answer(
                        RU.TOO_FAST.format(current=current, limit=limit)
                    )
                    return
...
async def back_to_menu(message: Message):
    async with interaction_context(message) as ctx:
        user = ctx.user
        session = ctx.session
        active = await get_active_order(session, user) if user else None
...
async def handle_click(message: Message, state: FSMContext):
    async with interaction_context(message, state=state, preload_stats=True) as ctx:
        user = ctx.user
        if not user:
            return
        session = ctx.session
        ...
        if not active:
            ...
            return
        stats = await ctx.ensure_stats()
        cp = stats.get("cp", 1)
        user.clicks_total += cp
        ctx.achievements.extend(await evaluate_achievements(session, user, {"clicks"}))
        ...
        if active.progress_clicks >= active.required_clicks:
            ...
            ctx.achievements.extend(
                await evaluate_achievements(session, user, {"orders", "level", "balance"})
            )
...
async def shop_buy_item(message: Message, state: FSMContext):
    ...
    async with interaction_context(message, state=state) as ctx:
        user = ctx.user
        if not user:
            return
        session = ctx.session
        ...
        if has:
            ...
            ctx.achievements.extend(await evaluate_achievements(session, user, {"items"}))
            equipment = await session.scalar(
                select(UserEquipment).where(
                    UserEquipment.user_id == user.id,
                    UserEquipment.slot == item.slot,
                )
            )
            auto_equipped = False
            if equipment and equipment.item_id is None:
            ...
            if next_item:
                ...
                bonus_str = (
                    f"≈+{int(proj_bonus * 100)}%"
                    if "_pct" in item.bonus_type
                    else f"≈+{int(proj_bonus)}"
                )
                next_hint = (
                    f"Следующий уровень (по формуле): {format_price(proj_price)}, {bonus_str}"
                )
            status_line = RU.ITEM_AUTO_EQUIPPED if auto_equipped else RU.ITEM_EQUIP_PROMPT
...
async def profile_show(message: Message, state: FSMContext):
    async with interaction_context(message, preload_stats=True) as ctx:
        user = ctx.user
        if not user:
            return
        session = ctx.session
...
async def profile_daily(message: Message):
    async with interaction_context(message) as ctx:
        ...
        ctx.achievements.extend(await evaluate_achievements(session, user, {"daily", "balance"}))
        await message.answer(
...
async def show_global_stats(message: Message):
    async with interaction_context(message) as ctx:
        ...
        if not user:
            return
        session = ctx.session
        top_rows, player_rank, total_players = await fetch_income_leaderboard(session, user.id)
        active = await get_active_order(session, user)
    markup = kb_profile_menu(has_active_order=bool(active))
    lines = [RU.STATS_HEADER, ""]
    for idx in range(1, 6):
        if idx <= len(top_rows):
            name, income = top_rows[idx - 1]
...
async def show_studio(message: Message, state: FSMContext):
    async with session_scope() as session:
        ...
        if user.level < 20:
            ...
            return
        prestige = await get_prestige_entry(session, user)
        gain = max(0, user.balance // 500)
        bonus = prestige.reputation
...
NavigationHandler = Callable[[Message, FSMContext], Awaitable[None]]
CANCEL_ROUTES: Dict[str, NavigationHandler] = {}
BACK_ROUTES: Dict[str, NavigationHandler] = {}


async def _nav_menu(message: Message, state: FSMContext) -> None:
    """Return user to the root menu and clear FSM state."""
...
async def _nav_orders_browsing(message: Message, state: FSMContext) -> None:
    """Show the orders browsing page."""

    await state.set_state(OrdersState.browsing)
    await _render_orders_page(message, state)


async def _nav_shop_boosts(message: Message, state: FSMContext) -> None:
    """Return to the boosts list."""

    await state.set_state(ShopState.boosts)
    await render_boosts(message, state)


async def _nav_shop_items(message: Message, state: FSMContext) -> None:
...
async def _nav_team(message: Message, state: FSMContext) -> None:
    """Return to the team list view."""

    await state.set_state(TeamState.browsing)
    await render_team(message, state)


async def _nav_wardrobe(message: Message, state: FSMContext) -> None:
    """Return to the wardrobe list."""

    await state.set_state(WardrobeState.browsing)
    await render_inventory(message, state)


async def _nav_shop_root(message: Message, state: FSMContext) -> None:
    """Show the shop root menu."""

    await state.set_state(ShopState.root)
    await message.answer(RU.SHOP_HEADER, reply_markup=kb_shop_menu())


async def _nav_upgrades(message: Message, state: FSMContext) -> None:
    """Show upgrades menu with appropriate visibility for the team section."""

    await state.clear()
    async with session_scope() as session:
        user = await get_user_by_tg(session, message.from_user.id)
        include_team = bool(user and user.level >= 2)
    await message.answer(
        RU.UPGRADES_HEADER,
        reply_markup=kb_upgrades_menu(include_team=include_team),
    )
...
async def _dispatch_navigation(
    action_map: Dict[str, NavigationHandler], message: Message, state: FSMContext
) -> None:
    """Resolve a navigation handler by FSM state or fallback to menu."""

    current = await state.get_state()
    if current is None:
        await _nav_menu(message, state)
        return
    handler = action_map.get(current)
    if handler:
...
CANCEL_ROUTES[TutorialState.step.state] = tutorial_skip
CANCEL_ROUTES[OrdersState.confirm.state] = _nav_orders_browsing
CANCEL_ROUTES[ShopState.confirm_boost.state] = _nav_shop_boosts
CANCEL_ROUTES[ShopState.confirm_item.state] = _nav_shop_items
CANCEL_ROUTES[TeamState.confirm.state] = _nav_team
CANCEL_ROUTES[WardrobeState.equip_confirm.state] = _nav_wardrobe
for cancel_state in {
    ...
    CANCEL_ROUTES.setdefault(cancel_state, _nav_menu)

BACK_ROUTES[TutorialState.step.state] = tutorial_skip
BACK_ROUTES[OrdersState.confirm.state] = _nav_orders_browsing
BACK_ROUTES[OrdersState.browsing.state] = _nav_menu
BACK_ROUTES[ShopState.confirm_boost.state] = _nav_shop_boosts
BACK_ROUTES[ShopState.confirm_item.state] = _nav_shop_items
BACK_ROUTES[ShopState.root.state] = _nav_upgrades
BACK_ROUTES[ShopState.boosts.state] = _nav_shop_root
BACK_ROUTES[ShopState.equipment.state] = _nav_shop_root
BACK_ROUTES[TeamState.confirm.state] = _nav_team
BACK_ROUTES[TeamState.browsing.state] = _nav_upgrades
BACK_ROUTES[WardrobeState.equip_confirm.state] = _nav_wardrobe
BACK_ROUTES[WardrobeState.browsing.state] = _nav_upgrades
for back_state in {
    SkillsState.picking.state,
    HellClientState.intro.state,
    HellClientState.step1.state,
    ...
    BACK_ROUTES.setdefault(back_state, _nav_menu)


@router.message(F.text == RU.BTN_CANCEL)
@safe_handler
async def cancel_any(message: Message, state: FSMContext):
    await _dispatch_navigation(CANCEL_ROUTES, message, state)


@router.message(F.text == RU.BTN_BACK)
@safe_handler
async def handle_back(message: Message, state: FSMContext):
    await _dispatch_navigation(BACK_ROUTES, message, state)
...
async def fallback_help(message: Message):
    ...
    markup = await build_main_menu_markup(tg_id=message.from_user.id)
    await message.answer(RU.FALLBACK_HELP, reply_markup=markup)
